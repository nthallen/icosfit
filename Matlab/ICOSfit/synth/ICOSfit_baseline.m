classdef ICOSfit_baseline
  properties
    nu % wavenumber index into vectors 
    vectors % functions of nu
    p_coeffs % number of polynomial coefficients: 3 => quadratic
    Ptype % 0: polynomial of x, 1: polynomial of nu
    PV % initial polynomical coefficients
    Pscale % Polynomial scale factor
  end
  methods
    function B = ICOSfit_baseline( file )
      fid = fopen( file, 'r' );
      if fid > 0
        ...
        elseif A(1) ~= 0 || A(2) ~= 1
          fclose(fid);
          error('MATLAB:huarp:readerr','Error: File is not in func_base_ptbnu format');
        else
        ...
        error('MATLAB:huarp:readerr','Cannot read file %s', file);
      end
    end
  end
end
