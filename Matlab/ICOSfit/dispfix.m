...
% select = [ 0 1 2 3 ];
n_line_params = S.n_line_params + S.n_abs_line_params;
n_base_params = S.n_base_params + S.n_abs_params;
vf = (1:S.n_lines*n_line_params) + S.n_input_params+2*n_base_params+S.n_lines*n_line_params;
% vf = v + n_base_params + n_line_params;
% scannum = fitdata(:,6);
nfit = length(S.scannum);
bv = S.fitdata(:,vf);
bv = [ reshape(bv', n_line_params, nfit*S.n_lines )' 2*ones(nfit*S.n_lines,1)];
bv = reshape(bv', S.n_lines*(n_line_params+1), nfit);
figure;
colormap([ 0 0 .75; 0 1 1; 0 0 0]);
nsubplot( 1,4,1,[1 3]);
h = image(S.scannum([1 nfit]), [1/(n_line_params+1) S.n_lines]+.5, bv+1 );
%set(h,'CDataMapping','Scaled');
set(gca, 'YTick', 1:S.n_lines);
xlabel('Scan Number');
ylabel('Line Number' );
